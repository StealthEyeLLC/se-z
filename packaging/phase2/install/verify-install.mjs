...
async function runAs(uid, gid, groups, executable, argv) {
  const groupArgs = groups.length ? [`--groups=${groups.join(',')}`] : ['--clear-groups'];
  return await command('/usr/bin/setpriv', [`--reuid=${uid}`, `--regid=${gid}`, ...groupArgs, executable, ...argv]);
}
...
try {
  ...
  const operatorHealth = await runAs(operatorUid, operatorGid, [localGroup], '/usr/local/bin/se-z', ['health']);
