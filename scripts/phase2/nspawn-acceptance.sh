...
const installedDefinitions=await import('file:///opt/se-z/current/libexec/kernel/definitions.mjs');const describe=(await client.call('sez.describe')).response;assert.equal(describe.result.product,'se-z');assert.equal(describe.result.activeOperations.length,installedDefinitions.ACTIVE_OPERATION_NAMES.length);assert.equal(describe.catalogDigest,installedDefinitions.CATALOG_DIGEST);check('exact installed catalog',{catalogDigest:describe.catalogDigest,operationCount:describe.result.activeOperations.length});
