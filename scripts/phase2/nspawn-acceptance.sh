...
const root=(await client.call('sez.exec',{argv:['/usr/bin/id','-u']},{idempotencyKey:'nspawn-root-proof'})).response;const rootDone=await terminal(root.jobId);const rootOut=await stream(root.jobId,'stdout');const rootErr=await stream(root.jobId,'stderr');assert.equal(rootDone.result.job.exitCode,0);assert.equal(rootOut.sha256,'9a271f2a916b0b261fe3913bc911e89392a7d04d47b2140745739e6de9551fcb');assert.equal(rootErr.bytes,0);check('uid-0 execution',{requestId:root.requestId,jobId:root.jobId,receiptId:rootDone.receipt.receiptId});
...
machinectl shell "$name" /opt/se-z/current/runtime/bin/node /root/candidate/post.mjs
machinectl shell "$name" /usr/local/sbin/se-z-verify-install --evidence /root/install-verify-after.json

mkdir -p "$work/evidence"
for f in install-verify-before.json nspawn-pre.json nspawn-post.json install-verify-after.json; do cp "$rootfs/root/$f" "$work/evidence/$f"; done
