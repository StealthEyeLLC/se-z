...
(cd "$root/package" && sha256sum -c SHA256SUMS --ignore-missing >/dev/null)
...
const lostKey='host-candidate-response-loss';const disconnected=await restartClient.call('sez.exec',{argv:['/bin/bash','-lc','sleep 1; printf response-loss-ok']},{idempotencyKey:lostKey,disconnectAfterSend:true});await new Promise(r=>setTimeout(r,1500));const resume=(await restartClient.call('sez.request.resume',{operation:'sez.exec',idempotencyKey:lostKey,payload:{argv:['/bin/bash','-lc','sleep 1; printf response-loss-ok']},target:'host'})).response;assert.equal(resume.error,null);assert.equal(resume.result.originalRequestId,disconnected.request.requestId);assert.ok(resume.result.originalResponse.jobId);check('deterministic response-loss resume',{requestId:disconnected.request.requestId,jobId:resume.result.originalResponse.jobId});
...
if [[ -n $evidence ]]; then
  /opt/node-v24.18.0-linux-x64/bin/node -e "const fs=require('fs');const p=process.argv[1];const d=JSON.parse(fs.readFileSync(p));d.cleanup={passed:true,unit:'$unit',rootRemoved:true};fs.writeFileSync(p,JSON.stringify(d,null,2)+'\\n')" "$evidence"
fi
